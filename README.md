...
迭代节奏按我自己的研究需求走，详见 [ROADMAP.md](ROADMAP.md)。

近期关注（**不承诺时间，按需推进**）：

- [ ] **多市场跨品种套利** — 黄金跨市场（XAUUSDT vs XAUUSD）价差监控面板（与 Variable-Global 平台联动）
- [ ] **本地晨报 / 复盘自动化** — faster-whisper 录音转写 → LLM 生成晨会纪要 → 一键入库
- [ ] **300+ 策展信息源聚合** — YouTube / Twitter / 公众号多 agent pipeline，按主题分组输出
- [ ] **私有因子库 + 回测** — 本地因子计算 + 向量化回测，不依赖外部回测框架
- [ ] **多端一致体验** — 移动端布局优化 + 桌面端 PWA 化

## 私有 / 隐私

本项目是**个人自用项目**，**不接收外部贡献、不对外分发**。

- **本地数据**（自选股 / 持仓 / 研报 / 研究记录）：**仅存本地部署目录**，不进仓库、不上传云端
- **公开快照**（股票池 JSON / 历史行情 CSV）：保留在 `data/` 目录可被 git 跟踪，是我**主动选择公开**的子集
- **WorkBuddy 记忆**（`.workbuddy/`）：**已加入 .gitignore**，仅本机可见
- **代码改动提交到自己的 `wuxingyuenan5-lgtm/vibe_research` 仓库**，origin 仍保留上游 `simonlin1212/Vibe-Research` 以便拉取同步

> 如果你 fork 或 clone 本仓库，请注意：除 `data/` 下的公开快照外，**所有运行时数据都是私有的**。要保留跨设备同步，请自行配置备份方案（不推荐把私密数据传上任何云端）。
...
<!-- TODO: 选填——如要致谢原作者 simonlin1212 / 上游数据源 / 用到的库 / 工具，简短一段即可（不超过 10 行）。 -->
