...
- **更新 / 上游**：仓库自带的本份是固定可用的数据快照，可一直使用；如需跟进最新端点 / 扩数据源，去原始开源仓库 `a-stock-data` 查看（仓库地址保留在各源 LICENSE 文件中）。
...
- **上游**：原始开源仓库 `global-stock-data`，仓库地址保留在其 LICENSE 文件中。
...
- **上游**：原始开源仓库 `investment-news`，仓库地址保留在其 LICENSE 文件中。
