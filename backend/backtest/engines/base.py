...
# 事件（RSSHub）与基本面（Tushare）两路增强**没有搬过来**：前者要自建服务、
# 后者要付费 key，而本产品的数据一律走自己的取数层。
# 它们改成**惰性 import**：config 没点名就永远不会被 import；点名了就在下面报清楚的错。
# 🔴 不做 try/except 静默跳过 —— 配了 event_feeds 却悄悄没有事件，
#    等于把"没接上"渲染成一个看着正常的回测结果。
...
def _event_feed_specs(config: Dict[str, Any]) -> List[FeedSpec]:
    ...
    try:
        from backtest.loaders.rsshub_events import feed_specs_from_config
    except ImportError as exc:  # 本产品没搬这一路
        raise RuntimeError(
            "config 里配了 event_feeds，但事件增强（RSSHub）没有随本产品发行 —— "
            "去掉 event_feeds，或自行提供 backtest.loaders.rsshub_events"
        ) from exc
